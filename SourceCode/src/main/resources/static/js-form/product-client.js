...
function clickAddToCartDetail(productId) {
	...
	if (amount <= 0) {
		alert("Invalid value. Amount must >= 1");
...
function setCartProducts(data) {
	...
	if (data.length > 0) {
		...
		document.getElementById("innerCart").innerHTML =
			...
			'</ul>' +
			'<p class="btn-control">' +
			'<a href="/cart-detail" class="btn view-cart">Detail</a>' +
			'<a href="/check-out"class="btn">Payment</a>' +
			'</p>';
		...
		document.getElementById("cartQtyProducts").innerHTML = Object.keys(data).length;
		document.getElementById("innerCart").innerHTML = '<p class="minicart-empty">There are no products in the cart</p>';
	}
...
function deleteProduct(productId, path) {
	...
	var conf = confirm("Arer you sure?");
	if (conf) {
...
function reviewProduct(productId) {
	...
	const regex = /[<|>|/|"|'|-|_|!|~|`|@|#|$|%|^|&|*|?|+|=]/;
	if (cmt.trim() == "") {
		alert("Please leave your review for the product");
	} else if (regex.test(cmt)) {
		alert("Only alphanumeric input is allowed");
	} else {
		$.ajax({
			type: "POST",
...
function clickSearch() {
	...
	if (type == -1) {
		alert("What do you want to search by??");
...
function setCartDetail(data) {
	...
	var strFooter = '<tr class="cart_item wrap-buttons">' +
		...
		'<a class="btn back-to-shop">Đến cửa hàng</a>' +
		'</td>' +
		'</tr>';
